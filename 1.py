...
class WorkWise:
    def __init__(self, root):
        ...
        self.Reward_button = tk.Button(self.progress_frame, text="Reward", command=self.get_reward, width=5, bg="#333333",
                                    fg="#ffffff")
        self.Reward_button.pack(side=tk.LEFT, padx=5)
        self.get_cur_level()
    ...
    def create_tabs(self):
        ...
        # Tab Goals
        self.queue_frame = ttk.Frame(self.notebook, style="TFrame")
    ...
    def get_reward(self):
        try:
            ...
            quick_edit_window.title("Reward")
            quick_edit_window.geometry("300x390")
            quick_edit_window.configure(bg="#222121")
            quick_edit_window.attributes('-topmost', True)
            # Create Listbox
            listbox = tk.Listbox(quick_edit_window, height=15, width=40, font=("Helvetica", 14), fg="#ffffff",
                                 bg="#222121", selectmode=tk.SINGLE)
            ...
            def delete_selected_item():
                try:
                    ...
                    if selected_index:
                        ...
                        updated_content = "<r|e|w|a|r|d>".join(listbox.get(0, tk.END))
                        conn = sqlite3.connect('setting.db')
                        cursor = conn.cursor()
                        cursor.execute('UPDATE setting SET history = ?', (updated_content,))
                        conn.commit()
                        conn.close()
                    else:
            ...
            def get_history():
                ...
                if result:
                    current_times, current_history = result
                    return current_history.split("<r|e|w|a|r|d>")
                return []

            # Populate Listbox with history items
            history_items = get_history()
            for item in history_items:
                listbox.insert(tk.END, item)

            ok_button = tk.Button(quick_edit_window, text="OK", command=close_window, font=("Helvetica", 14))
            ok_button.pack(side=tk.RIGHT, padx=10)

            delete_button = tk.Button(quick_edit_window, text="Delete", command=delete_selected_item,
                                      font=("Helvetica", 14))
            delete_button.pack(side=tk.RIGHT, padx=10)

            return
        except Exception as e:
            messagebox.showerror("Error", f"Failed to open reward window: {str(e)}")
            return
    ...
    def add_gift(self, task_name, type, reward = ""):
        # thêm vào history times+1 update progress bar
        conn = sqlite3.connect('setting.db')
        cursor = conn.cursor()

        # Fetch current times and history
        cursor.execute(f'SELECT history, {type} FROM setting')
        result = cursor.fetchone()

        if result:
            current_history, reward_list = result
            print("reward_list", reward_list)
            reward_list = reward_list.split(',')
            random_reward = random.choice(reward_list)
            print("Current:", current_history, random_reward )
            new_history = current_history + '\n' + f"{random_reward}-{task_name[:(len(task_name)//2)]} <r|e|w|a|r|d>\n"
        ...
        def tasks_done():
            try:
                ...
                if selected_item:
                    ...
                    self.add_gift(task_name,"task",reward)
                return
            except:
    ...
    def build_goals_tab(self):
        ...
        def goals_setting():
            try:
                quick_edit_window = tk.Toplevel(self.root)
                quick_edit_window.title("SETIING")
                quick_edit_window.geometry("300x300")
                quick_edit_window.configure(bg="#222121")
                quick_edit_window.attributes('-topmost', True)

                # Label and Entry for
                todo_label = tk.Label(quick_edit_window, text="Todo Random Gifts:", fg="#ffffff", bg="#222121")
                todo_label.pack(padx=10, pady=5)
                todo_entry = tk.Entry(quick_edit_window, width=50)
                todo_entry.pack(padx=10, pady=5)

                # Label and Entry for Task Gift
                task_label = tk.Label(quick_edit_window, text="Task Random Gifts:", fg="#ffffff", bg="#222121")
                task_label.pack(padx=10, pady=5)
                task_entry = tk.Entry(quick_edit_window, width=50)
                task_entry.pack(padx=10, pady=5)

                # Label and Entry for Goal Gift
                goal_label = tk.Label(quick_edit_window, text="Goal Random Gifts:", fg="#ffffff", bg="#222121")
                goal_label.pack(padx=10, pady=5)
                goal_entry = tk.Entry(quick_edit_window, width=50)
                goal_entry.pack(padx=10, pady=5)

                def comfirm_word_mean():
                    try:
                        conn = sqlite3.connect('setting.db')
                        cursor = conn.cursor()
                        new_todoreward = todo_entry.get().rstrip(',')
                        new_taskreward = task_entry.get().rstrip(',')
                        new_goalreward = goal_entry.get().rstrip(',')
                        cursor.execute('UPDATE setting SET todo =?, task=?, goal=?',
                                            (new_todoreward,new_taskreward ,new_goalreward,))
                        conn.commit()
                        conn.close()
                        quick_edit_window.destroy()
                        return
                ...
                def get_old_reward():
                    ...
                    # Fetch current times and history
                    cursor.execute('SELECT todo, task, goal FROM setting')
                    result = cursor.fetchone()
                    conn.close()

                    if result:
                        random_todo, random_task, random_goal = result
                        return random_todo, random_task, random_goal
                    return []

                random_todo, random_task, random_goal = get_old_reward()
                todo_entry.insert(0, random_todo)
                task_entry.insert(0, random_task)
                goal_entry.insert(0, random_goal)
        ...
        goals_open_button = tk.Button(goals_buttons_frame, text="Note", command=goals_open_note, width=10, bg="#333333",
                                      fg="#ffffff")
        goals_open_button.pack(side=tk.LEFT, padx=7)
        goals_done_button = tk.Button(goals_buttons_frame, text="Done", command=goals_done, width=10, bg="#333333",
                                      fg="#ffffff")
        goals_done_button.pack(side=tk.LEFT, padx=7)
        goals_edit_button = tk.Button(goals_buttons_frame, text="Edit", command=goals_edit, width=10, bg="#333333",
                                      fg="#ffffff")
        goals_edit_button.pack(side=tk.LEFT, padx=7)
        goals_delete_button = tk.Button(goals_buttons_frame, text="Delete", command=goals_delete, width=10,
                                        bg="#333333", fg="#ffffff")
        goals_delete_button.pack(side=tk.LEFT, padx=7)
        goals_setting_button = tk.Button(goals_buttons_frame, text="Setting", command=goals_setting, width=10,
                                        bg="#333333", fg="#ffffff")
        goals_setting_button.pack(side=tk.LEFT, padx=7)
        self.goals_load_tasks()
